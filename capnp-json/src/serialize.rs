...
use crate::enumerant_value;

use super::{field_name, read_annots, DataFormat, JsonAnnots};
use capnp::{dynamic_list, dynamic_struct, dynamic_value, Error, Result};

/// Serialize a dynamic struct reader into a JSON object.
///
/// ```
/// # fn main() -> capnp::Result<()> {
/// # use capnp::schema_capnp::value as my_capnp_struct;
/// # use capnp_json::serialize;
/// let mut message = capnp::message::Builder::new_default();
/// let mut root = message.init_root::<my_capnp_struct::Builder>();
/// root.set_text("hello world");
///
/// let json = serialize(root.reborrow_as_reader().into(), Default::default())?;
/// # Ok(())
/// # }
/// ```
...
fn serialize_value(
    ...
    match reader {
        ...
        dynamic_value::Reader::AnyPointer(r) => {
            if r.is_null() {
                Ok(Some(Value::Null))
            } else {
                opts.unsupported_result("anypointer")
            }
        }
...
fn serialize_enum(value: dynamic_value::Enum, opts: &Opts) -> Result<Option<Value>> {
    if let Some(enumerant) = value.get_enumerant()? {
        let value = enumerant_value(&enumerant)?;
